 //批量导出   
   ...
   $(function(){
		...
		$.ajax({
            ...
            success:function(data){
               if(data == "noLimit"){
          	     ...
          	     $.ajax({
          	    	 type:"post",
          	    	 url:"lininfo/infoAction_exportpartExcelByConditions.action", //批量导出的action
          	    	 data:jsonstr,
          	    	 success:function(data){
	...
	$("#exportNow").bind("click",function(){
		$.ajax({
            type:"post", 
            url:"usersAction_check_lineExport_power",
            success:function(data){
               if(data == "noLimit"){
                   alert("您无此权限！请您联系超级管理员");
                   right.src="noPower.jsp";
           	    ...
           	    // alert(checkboxs.eq(4).attr("id"));
           	     ...
           	     $.ajax({
           	    	 type:"post",
           	    	 url:"lininfo/infoAction_exportExcelByConditions.action", //导出全部页的action
           	    	 data:jsonstr,
           	    	 success:function(data){
           	    		 var json1=JSON.parse(data);
           		    		if(json1!=""){ 		
		...
		$.ajax({
            ...
            success:function(data){
               if(data == "noLimit"){
          	     ...
          	     var json1={"lineNum":$("#searchlineNum").val(),
          	    		 "lineOperator":$("#searchlineOperator").val(),
          	    		 "lineSystem":$("#searchlineSystem").val()};
          		   
          	     //发送请求
          	     $.ajax({
          	    	 type:"post",
          	    	 url:"lininfo/infoAction_exportAllExcelByConditions.action", //批量导出的action
          	    	 data:json1,
          	    	 success:function(data){
          	    		 var json1=JSON.parse(data);
          		    		if(json1!=""){ 		
